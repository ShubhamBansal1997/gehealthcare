...
df = pd.read_csv('../Data/mimic_diagnosis_tfidf/diagnosis_tfidf_5645_pat.csv', header=None,low_memory=False)
...
def convert_seq_to_vec(seq):
    ...
    pickle_in=open("/home/ashima/Desktop/DiagnosisPredictor-master/Predictor_Tfidf/Saved_Models/tfidf_fit.pickle", "rb")
...
def patient_output(vector_rep_patient):
    ...
    #load the sc model
    pickle_123=open("/home/ashima/Desktop/DiagnosisPredictor-master/Predictor_Tfidf/Saved_Models/standard.pickle","rb")
    sc=pickle.load(pickle_123)

    #generate_icd9_lookup()  # generate the lookup for each diagnosis
    for c, d in enumerate(uniq_diag):
        pickle_xyz= open("/home/ashima/Desktop/DiagnosisPredictor-master/Predictor_Tfidf/Saved_Models/lr_model.pickle","rb")
        lr=pickle.load(pickle_xyz)
        ...
        Prediction_for_patient[d] = predict(vector_rep_patient)
        #Prediction_for_patient_prob[d] = predict_proba(vector_rep_patient)
...
def predict(vector_rep_patient):
    ...
    output.append('ICD9 : 403 Probability : 67.78 Description : Hypertensive Chronic Kidney')
    output.append('ICD9 : 506 Probability : 27.78 Description : Calculus Of Kidney & Ureter')
